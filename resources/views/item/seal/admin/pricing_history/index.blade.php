            ...
            <div class="row">
                    ...
                    <ul id="accordion" class="nav nav-pills nav-stacked sidebar-menu">
                        ...
                        <li>
                            <li class="nav-item"><a class="nav-link"  href="#"><i class="fa fa-th-list"></i>&nbsp; Pricing History</a>
                                <ul class="sub">
                                    <li><a href="{{ route('admin_seal_pricing_history_create', $seal->id) }}"><i class="fa fa-th-list"></i>&nbsp;Pricing History List</a></li>
                ...
                <div class="col-lg-10 col-md-9 col-sm-9 col-xs-12 col-lg-offset-2 col-sm-offset-3 main">
                    <div class="row">
                        <div class="panel panel-default">
                            <div class="panel-heading">
                                {{ strtoupper($seal->name) }} PRICING HISTORY
                            </div>
                        </div>
                    </div>

                    @if(count($sealPricingHistory) != 0)
                        <div class="row">
                            <div class="col-lg-12">
                                <div class="col-lg-12">
                                    <div class="table-responsive">
                                        <table class="table">
                                            ...
                                            <tbody>
                                            @foreach($sealPricingHistory as $pricing_history)
                                                <tr>
                                                    <td>{{ ((($sealPricingHistory->currentPage() - 1) * $sealPricingHistory->perPage()) + ($ctr++) + 1) }}</td>
                                                    <td>{{ $pricing_history->po_number }}</td>
                                                    <td>{{ $pricing_history->pricing_date }}</td>
                                                    <td>{{ number_format($pricing_history->price, 2) }}</td>
                                                    ...
                                                    <td>{{ date('F d, Y', strtotime($pricing_history->created_at)) }}</td>
                                                </tr>
                                            @endforeach
                                            </tbody>
                                        </table>
                    ...
                    @else
                        <div class="row">
                            <div class="col-lg-12">
                                <div class="alert alert-danger" role="alert" style="background-color: #d9534f; border-color: #b52b27; color: white;">You have 0 records for {{ $seal->name }}'s Pricing History.</div>
                            </div>
                        </div>
                    @endif
                </div>
